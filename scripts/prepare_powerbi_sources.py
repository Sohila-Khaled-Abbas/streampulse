...
from src.extract.generate_dirty_training_datasets import run_all as generate_dirty_sources
...
def prepare_and_verify_all_sources(force_refresh_historical: bool = False) -> Dict[str, Any]:
    ...
    results = {
        "source_1_postgres_staging": {"status": "UNKNOWN", "records": 0, "path": "staging.stg_netflix_titles"},
        "source_2_historical_csv": {"status": "UNKNOWN", "records": 0, "path": "data/raw/netflix_enriched_historical.csv"},
        "source_3_imdb_ratings_csv": {"status": "UNKNOWN", "records": 0, "path": "data/raw/imdb_external_ratings.csv"},
        "source_4_viewership_parquet": {"status": "UNKNOWN", "records": 0, "path": "data/raw/streaming_viewership_wide.parquet"},
        "source_5_budget_json": {"status": "UNKNOWN", "records": 0, "path": "data/raw/boxoffice_budget_feed.json"},
    ...
    results["source_2_historical_csv"]["records"] = hist_integrity.get("total_records", 0)
    results["source_2_historical_csv"]["status"] = "OK" if hist_integrity.get("is_valid") else "FAILED"
    ...
    if os.path.exists(s3_path):
        ...
        required_cols_s3 = ["title_id", "user_score", "vote_count_raw", "critic_metascore", "snapshot_timestamp"]
        valid_s3 = all(c in df_s3.columns for c in required_cols_s3) and len(df_s3) > 0
        results["source_3_imdb_ratings_csv"]["status"] = "OK" if valid_s3 else "INVALID_COLUMNS"
    ...
    if os.path.exists(s4_path):
        ...
        required_cols_s4 = ["catalog_ref_id", "territory_region", "Hours_2026_01", "Hours_2026_02", "Hours_2026_03", "avg_completion_pct"]
        valid_s4 = all(c in df_s4.columns for c in required_cols_s4) and len(df_s4) > 0
        results["source_4_viewership_parquet"]["status"] = "OK" if valid_s4 else "INVALID_COLUMNS"
        results["source_4_viewership_parquet"]["records"] = len(df_s4)
        logger.info(f"[OK] Source 4 (Viewership Wide Parquet): {len(df_s4)} rows verified.")
    else:
    ...
    if os.path.exists(s5_path):
        with open(s5_path, "r", encoding="utf-8") as f:
            feed = json.load(f)
        items = feed.get("data", [])
        valid_s5 = len(items) > 0 and "stream_id" in items[0] and "categorization" in items[0]
        results["source_5_budget_json"]["status"] = "OK" if valid_s5 else "INVALID_STRUCTURE"
    ...
    if db_manager.test_connection():
        try:
            with db_manager.connect() as conn:
                ...
                stg_count = conn.execute(text("SELECT COUNT(*) FROM staging.stg_netflix_titles")).scalar() or 0
                results["source_1_postgres_staging"]["records"] = stg_count
                results["source_1_postgres_staging"]["status"] = "OK" if stg_count > 0 else "EMPTY"
                logger.info(f"[OK] Source 1 (PostgreSQL staging.stg_netflix_titles): {stg_count} rows in database.")
        except Exception as err:
            results["source_1_postgres_staging"]["status"] = f"ERROR: {err}"
    else:
        results["source_1_postgres_staging"]["status"] = "DB_OFFLINE (Ready for when Docker starts)"
        logger.info("PostgreSQL is currently offline; table schema and seed statements are ready in sql/01_staging.sql.")
    ...
    logger.info(f" 1. PostgreSQL Staging:      [{results['source_1_postgres_staging']['status']}] ({results['source_1_postgres_staging']['records']} rows)")
    logger.info(f" 2. Historical Catalog CSV:  [{results['source_2_historical_csv']['status']}] ({results['source_2_historical_csv']['records']} titles, 1945-2024)")
    logger.info(f" 3. IMDb Ratings CSV:        [{results['source_3_imdb_ratings_csv']['status']}] ({results['source_3_imdb_ratings_csv']['records']} snapshots)")
    logger.info(f" 4. Viewership Wide Parquet: [{results['source_4_viewership_parquet']['status']}] ({results['source_4_viewership_parquet']['records']} records)")
    logger.info(f" 5. Budget Feed JSON:        [{results['source_5_budget_json']['status']}] ({results['source_5_budget_json']['records']} items)")
    logger.info("================================================================================")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare and Validate all Power BI Multi-Source Datasets")
    parser.add_argument("--force-refresh-historical", action="store_true", help="Force redownload of Kaggle historical CSV")
    args = parser.parse_args()

    results = prepare_and_verify_all_sources(force_refresh_historical=args.force_refresh_historical)
    all_ok = (
    ...
    if all_ok:
        logger.info("[SUCCESS] All files and datasets are ready for Power BI Desktop ingestion!")
        sys.exit(0)
    else:
